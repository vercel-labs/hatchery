...
Use Braintrust to trace a Hatchery chat across its classifier, title generation, thread turns, sandbox, subagents, tools, and delivery. Each chat lazily creates one durable `hatchery.chat` root; later turns and serverless invocations append children to that same trace.
...
- `hatchery.thread.turn`, `hatchery.thread.tool`: thread model call and Hatchery tool
...
Correlate with `chat.id`, `agent.id`, `worker.id`, `task.id`, `command.id`, `event.id`, and `queue.message_id`. Check span errors, input/output, deployment ID, environment, and Git commit SHA.
