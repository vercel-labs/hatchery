...
Setup (route must point at this channel, /chat/v1/github):

    vercel connect create github --triggers
    vercel connect detach <uid> --yes
    vercel connect attach <uid> --triggers --trigger-path /chat/v1/github --yes
...
class GitHubChannel:
    ...
    async def handle(self, webhook: chat.Webhook, bus: chat.Bus) -> chat.Ack:
        ...
        if event_name not in ("issue_comment", "pull_request_review_comment") or payload.get("action") != "created":
            return chat.Ack(200, '{"ok": true, "ignored": true}')
    ...
    def _gate(self, event_name: str, payload: dict) -> chat.Inbound | None:
        ...
        return chat.Inbound(token=token, text=text, state=state)
