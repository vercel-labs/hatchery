...
from hatchery.store import auth as auth_store
from hatchery.store import chats, events, spaces, turns
...
async def start_thread(
    ...
    *,
    ...
    for _, receipt in reversed(await events.read(chat_id, "notifications")):
        ...
        if result and result.get("status") == "sent" and binding:
            await chats.bind(binding["token"], chat_id, provider, binding["state"])
        return result or {
            "status": "unknown",
    ...
    async with _client(provider) as client:
        ...
        async with turns.run(lock_key):
            ...
            try:
                ...
                binding = None
            receipt = {"key": digest, "result": result}
