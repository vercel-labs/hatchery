...
for svc in github slack; do
  vercel connect detach "$svc/hatchery" --yes
  if [ "$branch" = "main" ]; then
    vercel connect attach "$svc/hatchery" --triggers --trigger-path "/channels/v1/$svc" --yes
  else
    vercel connect attach "$svc/hatchery" --triggers --trigger-branch "$branch" --trigger-path "/channels/v1/$svc" --yes
  fi
done
