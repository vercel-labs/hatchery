...
for svc in github slack; do
  ...
  if [ "$branch" = "main" ]; then
    vercel connect attach "$svc/fabricator" --triggers --trigger-path "/channels/v1/$svc" --yes
  else
    vercel connect attach "$svc/fabricator" --triggers --trigger-branch "$branch" --trigger-path "/channels/v1/$svc" --yes
  fi
done
