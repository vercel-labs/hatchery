...
import chat
from chat.channels import github
...
class FakeBus:
    ...
    async def dispatch(self, inbound: chat.Inbound) -> chat.Session:
        ...
        return chat.Session(id="ses_x", token=f"github:{inbound.token}", channel="github", created_at="")
...
def forwarded(payload: dict, event: str, delivery: str = "d1", auth: str = "Bearer good") -> chat.Webhook:
    return chat.Webhook(
...
def sess(kind: str = "issue") -> chat.Session:
    ...
    return chat.Session(id="ses_x", token="github:t", channel="github", created_at="", channel_state=state)
...
async def test_reply_posts_issue_comment_with_marker():
    calls: list[httpx.Request] = []
    await api_channel(calls).on_event(chat.event(chat.protocol.MESSAGE_COMPLETED, message="ported!"), sess())
    [request] = calls
...
async def test_reply_to_review_thread_uses_replies_endpoint():
    calls: list[httpx.Request] = []
    await api_channel(calls).on_event(chat.event(chat.protocol.MESSAGE_COMPLETED, message="ok"), sess("review_thread"))
...
async def test_long_reply_is_chunked(monkeypatch):
    ...
    await api_channel(calls).on_event(chat.event(chat.protocol.MESSAGE_COMPLETED, message="x" * (size + 1)), sess())
    assert len(calls) == 2
