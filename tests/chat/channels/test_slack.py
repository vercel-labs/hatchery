...
class FakeBus:
    def __init__(self) -> None:
        self.dispatched: list[chat.Inbound] = []
        self.seen: set[str] = set()

    async def dispatch(self, inbound: chat.Inbound) -> chat.Session:
        self.dispatched.append(inbound)
        return chat.Session(id="ses_x", token=f"slack:{inbound.token}", channel="slack", created_at="")
...
def forwarded(payload: dict, auth: str = "Bearer good", extra_headers: dict | None = None) -> chat.Webhook:
    headers = {"authorization": auth}
    headers.update(extra_headers or {})
    return chat.Webhook(body=json.dumps(payload).encode(), headers=headers)
...
def sess() -> chat.Session:
    return chat.Session(
        id="ses_x", token="slack:C1:100.1", channel="slack", created_at="",
        channel_state={"channel_id": "C1", "thread_ts": "100.1"},
    )


async def test_turn_started_sets_typing_status_with_connect_token(connect_stub):
    calls: list[httpx.Request] = []
    await api_channel(calls).on_event(chat.event(chat.protocol.TURN_STARTED), sess())
    [request] = calls
...
async def test_reply_posts_into_thread():
    calls: list[httpx.Request] = []
    await api_channel(calls).on_event(chat.event(chat.protocol.MESSAGE_COMPLETED, message="done!"), sess())
    [request] = calls
...
async def test_status_is_truncated():
    calls: list[httpx.Request] = []
    await api_channel(calls).on_event(chat.event(chat.protocol.STATUS_UPDATED, status="x" * 80), sess())
...
async def test_turn_failed_posts_error():
    calls: list[httpx.Request] = []
    await api_channel(calls).on_event(chat.event(chat.protocol.TURN_FAILED, error="boom"), sess())
    params = dict(urllib.parse.parse_qsl(calls[0].read().decode()))
